...
export const EditUserSchema = z.object({
  ...
  user_role: z
    .nativeEnum(UserRole)
    .refine((value) => Object.values(UserRole).includes(value as UserRole), {
...
export const JetskiSchema = z.object({
  jetski_registration: z.string().regex(/^(?:[A-Z]{2})-(?:\d{3,6})$/, {
    message:
      "Registration must start with 2 letters representing a city followed by a hyphen and 3 to 6 numbers. For example: ST-123456",
  }),
  ...
  jetski_topSpeed: z
    ...
    .refine(
      ...
      {
        message:
          "Top speed must be a positive number up to 150, followed by 'mph' or 'km/h'.",
      }
    ),
  jetski_kW: z.string().refine(
    (value) => {
  ...
  jetski_manufacturingYear: z.string().refine(
    (value) => {
      return (
        value.length === 4 &&
    ...
    {
      message:
        "Manufacturing year must be a four-digit year between next one and 1950.",
    }
  ),
});
...
export const ReservationOptionSchema = z.object({
  ...
  rentalprice: z.string().refine(
    ...
    {
      message:
        "Rental price must be a valid numeric amount followed by a space and € symbol.",
    }
  ),
});
